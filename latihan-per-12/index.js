...
function setMetric(index, label, value, meta, icon) {
  ...
  metric.meta.textContent = meta ?? "Belum dimuat";
  metric.icon.className = `fa-solid ${icon}`;
}
...
function updateLayout(themeKey) {
  ...
  els.pageTitle.textContent = theme.title;
  els.pageSubtitle.textContent = theme.subtitle;
  els.breadcrumbTitle.textContent = theme.title;
  els.controlTitle.textContent = theme.controlTitle;
  els.controlSubtitle.textContent = theme.controlSubtitle;
  els.detailTitle.textContent = theme.detailTitle;
  els.detailSubtitle.textContent = theme.detailSubtitle;
  els.mainChartTitle.textContent = theme.mainChartTitle;
  els.sideChartTitle.textContent = theme.sideChartTitle;
  els.sideChartSubtitle.textContent = theme.sideChartSubtitle;
  els.submitBtn.innerHTML = `<i class="fa-solid ${theme.icon}"></i> ${theme.submitText}`;
  els.dynamicControls.innerHTML = controls[themeKey]();
  els.geoBtn.classList.toggle("hidden", themeKey !== "weather");
  els.mainChartBadge.textContent = themeKey === "covid" ? "Doughnut Chart" : "Line / Bar Chart";
  bindDynamicControls();
}

function renderLoadingTheme() {
  metricEls.forEach((metric, index) => {
    metric.label.textContent = `Data ${index + 1}`;
    metric.value.textContent = "-";
    metric.meta.textContent = "Memuat data...";
  });
  els.table.innerHTML = "";
  els.jsonPreview.textContent = "Memuat data...";
...
async function loadCurrency() {
  ...
  els.table.innerHTML = codes.map((code) => row(code, code === "IDR" ? formatMoney(data.rates[code]) : formatNumber(data.rates[code], { maximumFractionDigits: 4 }))).join("");
...
async function loadPopulation() {
  ...
  els.table.innerHTML = rows.slice(-6).reverse().map((item) => row(item.date, `${item.country.value}: ${formatNumber(item.value)}`)).join("");
...
async function loadSales() {
  ...
  els.table.innerHTML = products.slice(0, 8).map((product) => row(product.title, `${formatMoney(product.price, "USD")} | stok ${product.stock} | rating ${product.rating}`)).join("");
...
async function loadActiveTheme() {
  ...
  try {
    ...
    els.jsonPreview.textContent = JSON.stringify(state.lastPayload, null, 2);
    const updatedText = `Update terakhir: ${new Date().toLocaleString("id-ID")} | Refresh 30 detik`;
    els.lastUpdated.textContent = updatedText;
    els.topLastUpdated.innerHTML = `<i class="fa-regular fa-clock"></i> ${updatedText}`;
    ...
    els.table.innerHTML = row("Error", err.message, false);
    els.jsonPreview.textContent = JSON.stringify({ theme: state.activeTheme, error: err.message }, null, 2);
...
function bindSidebarSearch() {
  els.sidebarSearch.addEventListener("input", () => {
    ...
    els.menuSearchEmpty.classList.toggle("hidden", visibleCount > 0);
  });
...
els.refreshBtn.addEventListener("click", loadActiveTheme);
els.interval.addEventListener("change", resetTimer);
